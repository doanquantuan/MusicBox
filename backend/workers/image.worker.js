...
const imageWorker = new Worker(
    ...
    async (job) => {
        ...
        if (job.name === 'delete-image') {
            ...
            if (!imageUrl) {
                throw new Error("Image URL là bắt buộc");
            }
...
imageWorker.on('completed', (job, result) => {
    console.log(`[Image Worker] Job ${job.id} (${job.name}) completed.`);
});

imageWorker.on('failed', (job, err) => {
    console.error(`[Image Worker] Job ${job?.id} (${job?.name}) failed:`, err.message);
});
